...
import math
import datetime
from Color_Size import COLORS
from Color_Size import CENTER
from Color_Size import RADIUS
...
def tick():
    ...
    for i in range(0,360,6):
        x_cood = int(CENTER[0] + RADIUS * math.cos(i*math.pi / 180))
        y_cood = int(CENTER[1] + RADIUS * math.sin(i*math.pi / 180))

        hours_initial.append((x_cood,y_cood))

    for i in range(0,360,6):
        x_cood = int(CENTER[0] + (RADIUS-20) * math.cos(i*math.pi / 180))
        y_cood = int(CENTER[1] + (RADIUS-20) * math.sin(i*math.pi / 180))

        hours_destination.append((x_cood,y_cood))

    
    return hours_initial,hours_destination
